...
export default function RegolamentoContent({ initialRules }: { initialRules: Rule[] }) {
    const [activeFilter, setActiveFilter] = useState<RuleCategory>("Tutte");

    const filteredRules = initialRules.filter(rule =>
        activeFilter === "Tutte" ? true : (rule.category === activeFilter || (activeFilter === "Malus" && rule.points < 0))
    );
    ...
    const getIconForCategory = (category: string, points: number) => {
        if (points < 0 || category === "Malus") return <FiAlertTriangle className="text-red-500 mt-1" size={20} />;
        return <FiCheckCircle className="text-green-500 mt-1" size={20} />;
    };

    return (
        <main className="min-h-screen text-white p-6 md:p-12 pt-56 md:pt-44 pb-20 relative overflow-x-hidden">
                    ...
                    <AnimatePresence mode="popLayout">
                        {filteredRules.map((rule) => (
                            <motion.div
                                ...
                                key={rule.id}
                                className={`bg-[#131d36]/50 backdrop-blur-sm rounded-2xl p-6 border relative transition-all hover:border-oro/30 ${rule.points < 0 ? "border-red-900/50" : "border-gray-800"}`}
                            >
                                <div className="flex items-start justify-between gap-4">
                                    <div className="flex items-start gap-4">
                                        {getIconForCategory(rule.category, rule.points)}
                                        <div>
                                            <div className="flex flex-wrap items-center gap-3 mb-1">
                                                <h3 className="text-xl font-bold">{rule.title}</h3>
                                                <span className="text-[8px] font-black uppercase tracking-wider px-2 py-0.5 rounded-md bg-gray-800 text-gray-400">
                                                    {rule.category}
                                                </span>
                                            </div>
